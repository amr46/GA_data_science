...
master.train.index <-  sample(1:N, master.train.pct * N)  

# train.index <- sample(1:N, master.train.pct * N)       # random sample of records (training set)



for (index in 1:5){
train.index <- master.train.index[(((index-1)*30) + 1): (index*30)]
...
# results[index].plot <- results.plot + ggtitle(title)
