    ...
    for row in data:
        for index, items in enumerate(row):
            if(index == 5-offset):
                ...
                row[index] = AgeToGroup(row[index])
            if(items == "male"):
                row[index] = 0
            if(items == "female"):
                row[index] = 1
...
def runClassifiers(attributeName, data, dataIndices, resultIndex):

    random.shuffle(data)

    #Assume a default of 5x fold validation
    #data_len = len(data);
    ...
    (data_points, target_points) = create_data_and_outputs(data, dataIndices, resultIndex)
    
    data_points_array = np.array(data_points)
    target_array = np.array(target_points)

    titles = ["SVM Cross Validation Results", "Gaussian Naive Bayes", "Random Forests with 10 estimators" ]

    print "*** Start - Results for Attribute " + str(attributeName) + str(" ***")

    for i, clf in enumerate((svm_clf, gnb_clf, rf_clf)):
        scores = cross_validation.cross_val_score(clf, data_points_array, target_array, cv=5)
        print titles[i] + str(" Accuracy: %0.2f (+/- %0.2f)" % (scores.mean(), scores.std() / 2))
    
    print "*** End - Results for Attribute " + str(attributeName) + str(" ***")
    print ""


train_data  = create_csv2('titanic_train.csv', -1)
test_data   = create_csv2('titanic_train.csv', -1)
resultIndex = 1
...
dataIndices = set([2,4,5,6,7,9,10,11])
runClassifiers(str("All Attributes"), train_data, dataIndices, resultIndex)

dataTitles = ["Social Class", "Sex", "Age", "Has Siblings", "Has Parents", "Fare", "Cabin", "Embarked"]
count = 0; 
for attr in dataIndices:
    dataIndex = set([attr])
    runClassifiers(dataTitles[count], train_data, dataIndex, resultIndex)
    count = count + 1

print "### sex and age ###"
dataIndices = set([4,5])
runClassifiers(str("Sex and age"), train_data, dataIndices, resultIndex)
